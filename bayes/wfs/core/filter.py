...
class And(LogicalOperator):
    def __init__(self, and_items):
        super().__init__()
        self.items = and_items
...
class Or(LogicalOperator):
    ...
    def parse(cls, obj):
        ...
        for item in obj:
            for key in item:
                items.append(parse_fes(key, item[key]))
        return Or(items)
    ...
    def parse(cls, obj):
        ...
        for item in obj:
            ...
            if len(keys) == 1 and isinstance(item[keys[0]], list) and len(item[keys[0]]) == 2:
                val1 = parse_fes(keys[0], item[keys[0]][0])
                val2 = parse_fes(keys[0], item[keys[0]][1])
            elif len(keys) == 2:
